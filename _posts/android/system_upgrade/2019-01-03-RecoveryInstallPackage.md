...
main(int argc, char **argv) {
    ...
    load_volume_table();// 加载recovery分区表，详见2.1
    ...
    while ((arg = getopt_long(argc, argv, "", OPTIONS, NULL)) != -1) {
        ...
        case 'u': update_package = optarg; break;// 将optarg赋值给update_package，其实就是command里带的升级包路径
    ...
    if (update_package) {// 兼容老版本的cache路径
...
void load_volume_table()
{
    ...
    ret = fs_mgr_add_entry(fstab, "/tmp", "ramdisk", "ramdisk", 0);// 额外添加/tmp目录用于生成临时日志之类的  
    if (ret < 0 ) {
        ...
        if (arg != NULL && !strcmp(arg, "recovery")) {
            *argv = (char **) malloc(sizeof(char *) * MAX_ARGS);
            ...
            for (*argc = 1; *argc < MAX_ARGS; ++*argc) {
                if ((arg = strtok(NULL, "\n")) == NULL) break;
                (*argv)[*argc] = strdup(arg);// 把命令放到argv中带回到main函数里
            }
